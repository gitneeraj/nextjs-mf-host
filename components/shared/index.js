  ...
  createFederatedCatchAll() {
    const FederatedCatchAll = initialProps => {
      ...
      if (FederatedPage) {
        ...
        return <FederatedPage {...props} {...props.props} />;
      }
    ...
    FederatedCatchAll.getInitialProps = async ctx => {
      ...
      const matchedPage2 = await matchFederatedPage(ctx.asPath);
      const remote = matchedPage2?.value?.remote;
      const mod = matchedPage2?.value?.module;
      ...
      const FederatedPageWithGSSP = await container
        .get(mod)
        .then(factory => factory());

      if (FederatedPageWithGSSP.getServerSideProps) {
        const GSSP = await FederatedPageWithGSSP.getServerSideProps(ctx);
        console.log(GSSP);
        return { needsReload: true, ...props, ...GSSP, remotes };
      }

      if (!process.browser) {
        return { needsReload: true, ...props, remotes };
      }
      ...
      try {
        ...
        return { ...federatedPageProps, FederatedPage, matchedPage };
      } catch (err) {
        console.log('err', err);
        // TODO: Run getInitialProps for error page
        return { renderError: true, ...props };
