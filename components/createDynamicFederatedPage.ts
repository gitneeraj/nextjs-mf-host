...
function defaultPathResolver(path: string): ResolvedPathObject {
  ...
  const remoteContainer =
    slashIndex > -1 ? trimmedPath.substring(0, slashIndex) : trimmedPath;
  const modulePath =
    slashIndex > -1 ? trimmedPath.substring(slashIndex) : '/index';
...
export function createDynamicFederatedPage({
  ...
  const getRemoteModule = async (path: string) => {
    const { remoteContainer, modulePath } = await pathResolver(path);
    // const { remoteContainer, modulePath } = pathResolver(path);
    console.log(remoteContainer, modulePath);
    const remoteContainerGlobal =
    ...
    if (
      ...
      !(window as any)[remoteContainerGlobal]
    ) {
